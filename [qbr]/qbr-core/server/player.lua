...
local function CheckPlayerData(source, PlayerData)
    ...
    PlayerData.job.grade.name = PlayerData.job.grade.name or 'Indépendant'
    PlayerData.job.grade.level = PlayerData.job.grade.level or 0
    -- Labels and grade names always come from the shared jobs, so renaming/translating a job also updates existing characters
    local sharedJob = QBShared.Jobs[PlayerData.job.name]
    if sharedJob then
        PlayerData.job.label = sharedJob.label
        local sharedJobGrade = sharedJob.grades[tostring(PlayerData.job.grade.level)]
        if sharedJobGrade then PlayerData.job.grade.name = sharedJobGrade.name end
    end
    -- Gang
    PlayerData.gang = PlayerData.gang or {}
    PlayerData.gang.name = PlayerData.gang.name or 'none'
    PlayerData.gang.label = PlayerData.gang.label or 'Aucune affiliation'
    PlayerData.gang.isboss = PlayerData.gang.isboss or false
    PlayerData.gang.grade = PlayerData.gang.grade or {}
    PlayerData.gang.grade.name = PlayerData.gang.grade.name or 'none'
    PlayerData.gang.grade.level = PlayerData.gang.grade.level or 0
    local sharedGang = QBShared.Gangs[PlayerData.gang.name]
    if sharedGang then
        PlayerData.gang.label = sharedGang.label
        local sharedGangGrade = sharedGang.grades[tostring(PlayerData.gang.grade.level)]
        if sharedGangGrade then PlayerData.gang.grade.name = sharedGangGrade.name end
    end
    -- Other
    ...
    if source then
        if citizenid then
            ...
            if PlayerData and license == PlayerData.license then
                ...
                DropPlayer(source, 'Vous avez été expulsé pour exploitation de faille')
