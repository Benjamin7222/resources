...
function populateBanking(data)
{
    ...
    if(data.cardInformation !== undefined) {
        if (data.cardInformation.cardLocked == true) {
            ...
            $("#debitCardStatus").html('<div class="card-header">Carte bloquée</div><div class="card-body">Votre carte est actuellement BLOQUÉE.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="unLockCard">Débloquer la carte</button></div>');
        } else {
            $("#debitCardStatus").removeClass('bg-danger');
        $("#debitCardStatus").addClass('bg-success');
        $("#debitCardStatus").html('<div class="card-header">Carte active</div><div class="card-body">Votre carte est actuellement active.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="lockCard">Bloquer la carte</button></div>');
        }
...
$(function() {
    ...
    $("#initiateWithdraw").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#withdrawErrorMsg").html('Erreur lors du retrait : le montant n\'a pas été saisi ou n\'est pas un nombre positif');
        }
    });
    ...
    $("#initiateWithdrawATM").click(function() {
        ...
        if (currentLimit + parseInt(amount) <= Config.ATMTransLimit) {
            if(amount !== undefined && amount > 0) {
                ...
                $("#withdrawATMErrorMsg").html('Erreur lors du retrait : le montant n\'a pas été saisi ou n\'est pas un nombre positif');
            }
        } else {
            $("#withdrawATMErrorMsg").removeClass('alert-success').addClass('alert-danger');
            $("#withdrawATMError").css({"display":"block"});
    ...
    $("#initiateDeposit").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#depositErrorMsg").html('Erreur lors du dépôt : le montant n\'a pas été saisi ou n\'est pas un nombre positif');
        }
    });
    ...
    $("#initiateTransfer").click(function() {
        ...
        if(amount !== undefined && amount !== null && amount > 0 && sortcode !== undefined && sortcode !== null && sortcode > 0 && account !== undefined && account !== null && account > 0) {
            ...
            $("#transferError").css({"display":"block"});
            $("#transferErrorMsg").html('Erreur dans les informations saisies : vérifiez que le numéro de compte, le code guichet et le montant sont correctement remplis.');
        }
        ...
        if(number == "ENTER") {
            ...
            if (enteredPin !== null && enteredPin !== undefined || enteredPin.replace(/[^0-9]/g,"").length !== 4) {
                    if(clientPin == enteredPin) {
                        ...
                        $("#pinErrorMsg").html('Vous avez saisi un code PIN incorrect.');
