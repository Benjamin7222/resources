...
AddEventHandler('qbr-banking:savingsDeposit', function(amount)
    ...
    if tonumber(amount) <= currentBank then
        ...
        local savings = savingsAccounts[xPlayer.PlayerData.citizenid].AddMoney(tonumber(amount), 'Virement du compte courant vers le compte épargne')
...
AddEventHandler('qbr-banking:savingsWithdraw', function(amount)
    ...
    if tonumber(amount) <= currentSavings then
        local savings = savingsAccounts[xPlayer.PlayerData.citizenid].RemoveMoney(tonumber(amount), 'Virement du compte épargne vers le compte courant')
...
AddEventHandler('qbr-banking:createSavingsAccount', function()
    ...
    TriggerClientEvent('qbr-banking:openBankScreen', src)
    TriggerClientEvent('qbr-banking:successAlert', src, 'Vous avez ouvert un compte épargne avec succès.')
...
exports['qbr-core']:AddCommand('givecash', 'Donner de l\'argent à un joueur.', {{name = 'id', help = 'ID du joueur'}, {name = 'amount', help = 'Montant'}}, true, function(source, args)
    local src = source
				...
				if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(id))) < distance then
					if xPlayer.Functions.RemoveMoney('cash', amount) then
						if xReciv.Functions.AddMoney('cash', amount) then
							TriggerClientEvent('QBCore:Notify', src, 9, "Vous avez donné " .. tostring(amount) .. "$ à l'ID " .. tostring(id) .. ".", 2000, 0, 'hud_textures', 'check')
							TriggerClientEvent('QBCore:Notify', id, 9, "Vous avez reçu " .. tostring(amount) .. "$ de l'ID " .. tostring(src), 2000, 0, 'hud_textures', 'check')
							TriggerClientEvent("payanimation", src)
						else
							TriggerClientEvent('QBCore:Notify', src, 9, "Impossible de donner l'argent à cet ID.", 2000, 0, 'mp_lobby_textures', 'cross')
						end
					else
						TriggerClientEvent('QBCore:Notify', src, 9, "Vous n'avez pas cette somme.", 2000, 0, 'mp_lobby_textures', 'cross')
					end
				else
					TriggerClientEvent('QBCore:Notify', src, 9, "Vous êtes trop loin.", 2000, 0, 'mp_lobby_textures', 'cross')
				end
			else
				TriggerClientEvent('QBCore:Notify', src, 9, "Vous êtes mort.", 2000, 0, 'mp_lobby_textures', 'cross')
			end
		else
			TriggerClientEvent('QBCore:Notify', src, 9, "ID incorrect.", 2000, 0, 'mp_lobby_textures', 'cross')
		end
	else
		TriggerClientEvent('QBCore:Notify', src, 9, "Utilisation : /givecash [ID] [MONTANT]", 2000, 0, 'mp_lobby_textures', 'cross')
	end
end)
