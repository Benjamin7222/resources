...
RegisterNetEvent('QBCore:Command:GoToMarker', function()
    ...
    if not IsWaypointActive() then
        Notify(9, 'Aucun point de repère défini', 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        return 'marker'
    end
    ...
    if not found then
        ...
        Notify(9, 'Erreur lors de la téléportation', 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end

    -- If Z coord was found, set coords in found coords.
    SetEntityCoords(ped, x, y, groundZ)
    Notify(9, 'Téléporté', 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
end)
...
RegisterNetEvent('QBCore:Command:SpawnHorse', function(HorseName)
    ...
    if hash then
		...
		Notify(9, 'Modèle introuvable', 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
