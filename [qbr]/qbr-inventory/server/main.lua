		...
		if name and id then
			...
			if name == "stash" then
				...
				if Stashes[id] and Stashes[id].isOpen then
					...
					secondInv.label = Lang:t("info.stash_none")
			...
			elseif name == "otherplayer" then
				...
				if OtherPlayer then
					...
					secondInv.label = Lang:t("info.playerLabel")..id
	...
	elseif exports['qbr-core']:SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif exports['qbr-core']:SplitStr(shopType, "_")[1] == "Itemshop" then
			...
			elseif bankBalance >= price then
				...
				TriggerClientEvent('QBCore:Notify', src, 9, "Vous n'avez pas assez d'argent liquide..", 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
			end
		else
...
exports['qbr-core']:AddCommand("resetinv", "Réinitialiser l'inventaire (Admin)", {{name="type", help="stash"},{name="id/plate", help="ID du coffre ou plaque d'immatriculation"}}, true, function(source, args)
...
exports['qbr-core']:AddCommand("giveitem", "Donner un objet (Admin)", {{name="id", help="ID du joueur"},{name="item", help="Nom de l'objet (pas son libellé)"}, {name="amount", help="Quantité d'objets"}}, true, function(source, args)
