...
RegisterNetEvent('qbr-bossmenu:client:OpenMenu', function()
    ...
    local bossMenu = {
        {
            header = "Menu patron - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
        },
        {
            header = "📋 Gérer les employés",
            txt = "Consulter la liste des employés",
            params = {
                event = "qbr-bossmenu:client:employeelist",
            }
        },
        {
            header = "💛 Embaucher des employés",
            txt = "Embaucher des civils à proximité",
            params = {
        ...
        {
            header = "🚪 Tenues",
            txt = "Voir les tenues enregistrées",
            params = {
        ...
        },
        {
            ...
            txt = "Consulter le solde de la société",
            params = {
...
RegisterNetEvent('qbr-bossmenu:client:employeelist', function()
    local EmployeesMenu = {
        {
            header = "Gérer les employés - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
        },
    }
...
RegisterNetEvent('qbr-bossmenu:client:ManageEmployee', function(data)
    local EmployeeMenu = {
        {
            header = "Gérer " .. data.player.name .. " - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
        },
    }
...
RegisterNetEvent('qbr-bossmenu:client:HireMenu', function()
    local HireMenu = {
        {
            header = "Embaucher des employés - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
        },
    }
...
RegisterNetEvent('qbr-bossmenu:client:SocietyMenu', function()
    exports['qbr-core']:TriggerCallback('qbr-bossmenu:server:GetAccount', function(cb)
        local SocietyMenu = {
            {
                header = "Solde : $" .. comma_value(cb) .. " - " .. string.upper(PlayerJob.label),
                isMenuHeader = true,
            },
            {
                header = "💸 Déposer",
                txt = "Déposer de l'argent sur le compte",
                params = {
            ...
            {
                header = "💸 Retirer",
                txt = "Retirer de l'argent du compte",
                params = {
...
RegisterNetEvent('qbr-bossmenu:client:SocetyDeposit', function(money)
    local deposit = exports['qbr-input']:ShowInput({
        header = "Déposer de l'argent <br> Solde disponible : $" .. money,
...
RegisterNetEvent('qbr-bossmenu:client:SocetyWithDraw', function(money)
    local withdraw = exports['qbr-input']:ShowInput({
        header = "Retirer de l'argent <br> Solde disponible : $" .. money,
        submitText = "Confirmer",
        inputs = {
            {
                type = 'number',
        ...
        for k, v in pairs(Config.Jobs) do
            if k == PlayerJob.name and PlayerJob.isboss then
                if #(pos - v) < 5.0 then
                    inRangeBoss = true
                    if #(pos - v) <= 3.0 then
                        if not shownBossMenu then DrawText3D(v, "~b~E~w~ - Ouvrir la gestion du métier") end
                        nearBossmenu = true
