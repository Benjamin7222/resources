...
RegisterNetEvent('qbr-gangmenu:client:OpenMenu', function()
    ...
    local gangMenu = {
        {
            header = "Gestion du gang - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
        },
        {
            header = "📋 Gérer les membres du gang",
            txt = "Recruter ou renvoyer des membres du gang",
            params = {
                event = "qbr-gangmenu:client:ManageGang",
            }
        },
        {
            header = "💛 Recruter des membres",
            txt = "Recruter des membres du gang",
            params = {
                event = "qbr-gangmenu:client:HireMembers",
            }
        },
        {
            header = "🗄️ Accès au stockage",
            txt = "Ouvrir le coffre du gang",
            params = {
        ...
        {
            header = "🚪 Tenues",
            txt = "Changer de tenue",
            params = {
                event = "qbr-gangmenu:client:Warbobe",
            }
        },
        {
            header = "💰 Gestion de l'argent",
            txt = "Consulter le solde du gang",
            params = {
...
RegisterNetEvent('qbr-gangmenu:client:ManageGang', function()
    local GangMembersMenu = {
        {
            header = "Gérer les membres du gang - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
        },
    }
...
RegisterNetEvent('qbr-gangmenu:lient:ManageMember', function(data)
    local MemberMenu = {
        {
            header = "Gérer " .. data.player.name .. " - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
        },
    }
...
RegisterNetEvent('qbr-gangmenu:client:HireMembers', function()
    local HireMembersMenu = {
        {
            header = "Recruter des membres du gang - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
        },
    }
        ...
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMembersMenu[#HireMembersMenu + 1] = {
                    header = v.name,
                    txt = "ID citoyen : " .. v.citizenid .. " - ID : " .. v.sourceplayer,
                    params = {
                        isServer = true,
...
RegisterNetEvent('qbr-gangmenu:client:SocietyMenu', function()
    exports['qbr-core']:TriggerCallback('qbr-gangmenu:server:GetAccount', function(cb)
        local SocietyMenu = {
            {
                header = "Solde : $" .. comma_valueGang(cb) .. " - " .. string.upper(PlayerGang.label),
                isMenuHeader = true,
            },
            {
                header = "💸 Déposer",
                txt = "Déposer de l'argent",
                params = {
...
RegisterNetEvent('qbr-gangmenu:client:SocietyDeposit', function(saldoattuale)
    local deposit = exports['qbr-input']:ShowInput({
        header = "Déposer de l'argent <br> Solde disponible : $" .. saldoattuale,
...
RegisterNetEvent('qbr-gangmenu:client:SocietyWithdraw', function(saldoattuale)
    local withdraw = exports['qbr-input']:ShowInput({
        header = "Retirer de l'argent <br> Solde disponible : $" .. saldoattuale,
        submitText = "Confirmer",
        inputs = {
            {
                type = 'number',
        ...
        for k, v in pairs(Config.Gangs) do
            if k == PlayerGang.name and PlayerGang.isboss then
                if #(pos - v) < 5.0 then
                    inRangeGang = true
                    if #(pos - v) <= 1.5 then
                        if not shownGangMenu then DrawText3DGang(v, "~b~E~w~ - Ouvrir la gestion du gang") end
                        nearGangmenu = true
